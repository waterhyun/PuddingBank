    ...
    <div class="options-section">
      ...
      <div class="options-table">
        <table>
          ...
          <tbody>
            <tr v-for="option in groupedOptions" :key="`${option.rpay_type}-${option.lend_rate_type}`">
...
const groupedOptions = computed(() => {
  ...
  // 중복 제거를 위한 키 생성
  const uniqueKey = option => `${option.rpay_type}-${option.lend_rate_type}`
  const uniqueOptions = new Map()
  
  loan.value.options.forEach(option => {
    const key = uniqueKey(option)
...
const fetchLoanDetail = async () => {
  ...
  try {
    const response = await axios({
      method: 'get',
      url: `http://127.0.0.1:8000/api/v1/products/lease/${route.params.id}`
    })
    loan.value = {
      ...response.data,
      loan_type: '전세자금대출'
    ...
    console.error('Error fetching lease loan:', err)
